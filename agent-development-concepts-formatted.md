...
**-** **\*\*How It Works Under the Hood:\*\***It relies on a continuous prompt-response loop (often called a ReAct loop: Reason **$\\**rightarrow**$** Act **$\\**rightarrow**$** Observe). The system prompt instructs the model to format its thoughts and actions in a specific parsable syntax (like JSON or XML tool calls).
...
**-** **\*\*Real-World Analogy:\*\*** An assembly line. Agent 1 (Researcher) gathers data **$\\**rightarrow**$** passes it to Agent 2 (Writer) to draft a report **$\\**rightarrow**$** passes it to Agent 3 (Editor) to fix grammar.
...
**-** **\*\*What it is:\*\*** Lifecycle hooks triggered at specific moments (e.g., on\_agent\_start,
